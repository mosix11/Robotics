...
def diff_variable_elem_rot(rotmat, dt):
    res = diff(rotmat, dt)
    return simplify(factor(res))

def get_omega_from_rotmat(rotmat, dt):
    d_rotmat = diff_variable_elem_rot(rotmat, dt)
...
def find_omega_from_EULER_angles(phi, theta, psi, dt, seq):
    rotmat = MO.EULER_to_rotmat(phi, theta, psi, seq)
    w = get_omega_from_rotmat(rotmat, dt)
    return simplify(factor(w))

def find_omega_from_RPY_angles(phi, theta, psi, dt, seq):
